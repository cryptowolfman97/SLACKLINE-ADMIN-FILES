 ...
 * License Block"). This is a direct port of the same SHVLicense engine
 * used across SHV apps (Slackline etc.) — device-bound, tier-aware
 * ("pro" / "pro_plus"), with live revocation checks against the same
 * kl_licenses / kl_revocations / kl_products Supabase tables.
 *
 * The private signing key never appears here — only the public key baked
 * in from the Product Authority record. Signing happens exclusively in
 * Kotlin Apps Manager.
 */
object SupaLicense {
    ...
    fun isProductActive(productId: String): Boolean {
        ...
        } catch (e: Exception) { true } // fail-open on network error, verified offline below
    }
    ...
    fun checkLicense(code: String, context: Context, checkRevocation: Boolean = false): LicenseResult {
        ...
        return try {
            ...
            // Device-bound: license only works on the device it was generated for
